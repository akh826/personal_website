...
createApp({
  template: `
    <Renderer ref="renderer" pointer resize="window">
      ...
      <Scene>
        <Points ref="points" :position="{ z: -10 }">
          ...
          <ShaderMaterial :blending="2" :depth-test="false" :uniforms="uniforms" :vertex-shader="vertexShader" :fragment-shader="fragmentShader">
            <Texture src="css/index/sprite.png" uniform="uTexture" />
          </ShaderMaterial>
        </Points>
      </Scene>
      <EffectComposer>
  ...
  setup() {
    ...
    const palette = ["#69d2e7","#69d2e7","#69d2e7","#69d2e7","#69d2e7"]
  ...
  mounted() {
    ...
    renderer.onBeforeRender(() => {
      ...
      this.uniforms.uTime.value += this.clock.getDelta() * this.timeCoef * 40
